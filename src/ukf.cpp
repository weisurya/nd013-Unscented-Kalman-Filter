#include "ukf.h"
#include "Eigen/Dense"
#include <iostream>

using namespace std;
using Eigen::MatrixXd;
using Eigen::VectorXd;
using std::vector;

/**
 * Initializes Unscented Kalman filter
 * This is scaffolding, do not modify
 */
UKF::UKF() {
  // if this is false, laser measurements will be ignored (except during init)
  use_laser_ = true;

  // if this is false, radar measurements will be ignored (except during init)
  use_radar_ = true;

  // initial state vector
  x_ = VectorXd(5);

  // initial covariance matrix
  P_ = MatrixXd(5, 5);

  // Process noise standard deviation longitudinal acceleration in m/s^2
  // std_a_ = 30;
  std_a_ = 0.5;

  // Process noise standard deviation yaw acceleration in rad/s^2
  // std_yawdd_ = 30;
  std_yawdd_ = 0.6;
  
  //DO NOT MODIFY measurement noise values below these are provided by the sensor manufacturer.
  // Laser measurement noise standard deviation position1 in m
  std_laspx_ = 0.15;

  // Laser measurement noise standard deviation position2 in m
  std_laspy_ = 0.15;

  // Radar measurement noise standard deviation radius in m
  std_radr_ = 0.3;

  // Radar measurement noise standard deviation angle in rad
  std_radphi_ = 0.03;

  // Radar measurement noise standard deviation radius change in m/s
  std_radrd_ = 0.3;
  //DO NOT MODIFY measurement noise values above these are provided by the sensor manufacturer.
  
  /**
  TODO:

  Complete the initialization. See ukf.h for other member properties.

  Hint: one or more values initialized above might be wildly off...
  */
  // Set initialization to false, set to true in first call of ProcessMeasurement
  is_initialized_ = false;

  // Set initial state dimension
  n_x_ = 5;

  // Set initial augmented state dimension 
  n_aug_ = 7;

  // Define lambda
  lambda_ = 3 - n_aug_;

  // Set number of sigma points
  n_sigma_points_ = 2 * n_aug_ + 1;

  // Set vector for weights
  weights_ = VectorXd(n_sigma_points_);
  weights_(0) = lambda_ / (lambda_ + n_aug_);
  for(int i=1; i<n_sigma_points_; i++){
    weights_(i) = 0.5 / (n_aug_ + lambda_);
  }

  // Predicted sigma points matrix
  Xsig_pred_ = MatrixXd(n_x_, n_sigma_points_);

  // Initialize Normalize Innovation Squared (NIS) value for both sensors
  NIS_radar_ = 0.;
  NIS_lidar_ = 0.;

  // Sensor measurement size
  n_z_radar = 3; // rho, phi, r_dot
  n_z_lidar = 2; // px, py

  // Measurement covariance matrices
  R_radar_ = MatrixXd(n_z_radar, n_z_radar);
  R_lidar_ = MatrixXd(n_z_lidar, n_z_lidar);

  // Set initialize previous timestamp
  previous_timestamp_ = 0;
}

UKF::~UKF() {}

/**
 * @param {MeasurementPackage} meas_package The latest measurement data of
 * either radar or laser.
 */
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  /**
  TODO:

  Complete this function! Make sure you switch between lidar and radar
  measurements.
  */
  if(!is_initialized_){
    // First measurement
    if(meas_package.sensor_type_ == MeasurementPackage::RADAR){
      // Convert RADAR from polar to cartesian coordinates and initialize state
      float rho = meas_package.raw_measurements_[0];
      float phi = meas_package.raw_measurements_[1];
      float r_dot = meas_package.raw_measurements_[2];

      // Convert from polar to cartesian coordinates
      float px = rho * cos(phi);
      if(fabs(px) < 0.0001){
        cout << "Initialize px is too small. Adjust it into 0.1" << endl;
        px = 0.1;
      }

      float py = rho * sin(phi);
      if(fabs(py) < 0.0001){
        cout << "Initialize py is too small. Adjust it into 0.1" << endl;
        py = 0.1;
      }

      // Initialize state
      x_ << px, py, r_dot, 0.0, 0.0;

    } else if(meas_package.sensor_type_ == MeasurementPackage::LASER){
      // Extract value from raw measurement
      float px = meas_package.raw_measurements_[0];
      float py = meas_package.raw_measurements_[1];

      // Initialize state
      x_ << px, py, 0.0, 0.0, 0.0;
    }

    // Initialize state covariance matrix
    P_ = MatrixXd::Identity(n_x_, n_x_);

    // Update last measurement
    previous_timestamp_ = meas_package.timestamp_;

    // Done initializing, no need to predict/update
    is_initialized_ = true;
    cout << "Initialized!" << endl;
    return;
  }

  // Compute elapsed time from last measurement
  float dt = (meas_package.timestamp_ - previous_timestamp_) / 1000000.0;

  // Update last measurement
  previous_timestamp_ = meas_package.timestamp_;

  Prediction(dt);

  if(meas_package.sensor_type_ == MeasurementPackage::RADAR){
    cout<<"Sensor: RADAR"<<endl;
    UpdateRadar(meas_package);
  } else {
    cout<<"Sensor: LIDAR"<<endl;
    UpdateLidar(meas_package);
  }
}

MatrixXd UKF::GenerateSigmaPoints(double delta_t){
  // Create augmented mean vector
  VectorXd x_aug = VectorXd(n_aug_);
  x_aug.head(5) = x_;
  x_aug(5) = 0;
  x_aug(6) = 0;

  // Create augmented state covariance
  MatrixXd P_aug = MatrixXd(n_aug_, n_aug_);
  P_aug.fill(0.0);
  P_aug.topLeftCorner(5, 5) = P_;
  P_aug(5, 5) = std_a_ * std_a_;
  P_aug(6, 6) = std_yawdd_ * std_yawdd_;

  // Create square root matrix
  MatrixXd L = P_aug.llt().matrixL();

  // Create sigma point matrix
  MatrixXd Xsig_aug = MatrixXd(n_aug_, n_sigma_points_);
  Xsig_aug.col(0) = x_aug;
  for(int i = 0; i < n_aug_; i++){
    Xsig_aug.col(i + 1) = x_aug + sqrt(lambda_ + n_aug_) * L.col(i);
    Xsig_aug.col(i + 1 + n_aug_) = x_aug - sqrt(lambda_ + n_aug_) * L.col(i);
  }

  // Predict sigma points
  for(int i = 0; i < n_sigma_points_; i++){
    // Extract values for better readability
    double p_x = Xsig_aug(0, i);
    double p_y = Xsig_aug(1, i);
    double v = Xsig_aug(2, i);
    double yaw = Xsig_aug(3, i);
    double yawd = Xsig_aug(4, i);
    double nu_a = Xsig_aug(5, i);
    double nu_yawdd = Xsig_aug(6, i);

    // Predict state values
    double px_p, py_p;
    
    if (fabs(p_x) < 0.001 && fabs(p_y) < 0.001) {
			p_x = 0.1;
			p_y = 0.1;
		}

    // Avoid division by zero
    if(fabs(yawd) > 0.001){
      px_p = p_x + v / yawd * (sin(yaw + yawd * delta_t) - sin(yaw));
      py_p = p_y + v / yawd * (cos(yaw) - cos(yaw + yawd * delta_t));
    } else {
      px_p = p_x + v * delta_t * cos(yaw);
      py_p = p_y + v * delta_t * sin(yaw);
    }

    double v_p = v;
    double yaw_p = yaw + yawd * delta_t;
    double yawd_p = yawd;

    // Add noise
    px_p += 0.5 * nu_a * delta_t * delta_t * cos(yaw);
    py_p += 0.5 * nu_a * delta_t * delta_t * sin(yaw);
    v_p += nu_a * delta_t;
    yaw_p += 0.5 * nu_yawdd * delta_t * delta_t;
    yawd_p += nu_yawdd * delta_t;

    // Write predicted sigma point into right column
    Xsig_pred_(0, i) = px_p;
    Xsig_pred_(1, i) = py_p;
    Xsig_pred_(2, i) = v_p;
    Xsig_pred_(3, i) = yaw_p;
    Xsig_pred_(4, i) = yawd_p;
  }
  cout << "Sigma point generated!" << endl;
  return Xsig_pred_;
}

/**
 * Predicts sigma points, the state, and the state covariance matrix.
 * @param {double} delta_t the change in time (in seconds) between the last
 * measurement and this one.
 */
void UKF::Prediction(double delta_t) {
  /**
  TODO:

  Complete this function! Estimate the object's location. Modify the state
  vector, x_. Predict sigma points, the state, and the state covariance matrix.
  */
  // Generate sigma points
  MatrixXd Xsig_pred_ = GenerateSigmaPoints(delta_t);

  //Predicted state mean
  x_.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    x_ += weights_(i) * Xsig_pred_.col(i);
  }

  // Predicted state covariance matrix
  P_.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    // State difference
    VectorXd x_diff = Xsig_pred_.col(i) - x_;
    
    // Angle normalization
    while(x_diff(3) > M_PI){
      x_diff(3) -= 2. * M_PI;
    }
    while(x_diff(3) < -M_PI){
      x_diff(3) += 2. * M_PI;
    }

    P_ += weights_(i) * x_diff * x_diff.transpose();
  }
  cout<< "Predicted!" << endl;
}

/**
 * Updates the state and the state covariance matrix using a laser measurement.
 * @param {MeasurementPackage} meas_package
 */
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  /**
  TODO:

  Complete this function! Use lidar data to update the belief about the object's
  position. Modify the state vector, x_, and covariance, P_.

  You'll also need to calculate the lidar NIS.
  */
  // Create matrix for sigma points in measurement space
  cout<<"Create matrix for sigma points in measurement space"<<endl;
  MatrixXd Zsig = MatrixXd(n_z_lidar, n_sigma_points_);
  
  // Transform sigma points into measurement space
  cout<<"Transform sigma points into measurement space"<<endl;
  for(int i = 0; i < n_sigma_points_; i++){
    Zsig(0, i) = Xsig_pred_(0, i);
    Zsig(1, i) = Xsig_pred_(1, i);
  }

  // Mean predicted measurement
  cout<<"Mean predicted measurement"<<endl;
  VectorXd z_pred = VectorXd(n_z_lidar);
  z_pred.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    z_pred += weights_(i) * Zsig.col(i);
  }

  // Innovation covariance matrix S
  cout<<"Innovation covariance matrix S"<<endl;
  MatrixXd S = MatrixXd(n_z_lidar, n_z_lidar);
  S.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    // Residual
    cout<<"Residual"<<endl;
    VectorXd z_diff = Zsig.col(i) - z_pred;

    // Angle normalization
    cout<<"Angle normalization 1"<<endl;
    while(z_diff(1) > M_PI) z_diff(1) -= 2. * M_PI;
    while(z_diff(1) < -M_PI) z_diff(1) += 2. * M_PI;

    S += weights_(i) * z_diff * z_diff.transpose();
  }

  // Add measurement noise covariance matrix
  cout<<"Add measurement noise covariance matrix"<<endl;
  R_lidar_ << std_laspx_ * std_laspx_, 0, 0, std_laspy_ * std_laspy_;
  
  S += R_lidar_;

  // Parse lidar measurement
  cout<<"Parse lidar measurement"<<endl;
  VectorXd z = VectorXd(n_z_lidar);
  z <<  meas_package.raw_measurements_[0], meas_package.raw_measurements_[1];

  // Compute cross correlation matrix
  cout<<"Compute cross correlation matrix"<<endl;
  MatrixXd Tc = MatrixXd(n_x_, n_z_lidar);
  Tc.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    // Residual
    cout<<"Residual"<<endl;
    VectorXd z_diff = Zsig.col(i) - z_pred;

    // Angle normalization
    cout<<"Angle normalization"<<endl;
    while(z_diff(1) > M_PI) z_diff(1) -= 2. * M_PI;
    while(z_diff(1) < -M_PI) z_diff(1) += 2. * M_PI;

    // State difference
    cout<<"State difference"<<endl;
    VectorXd x_diff = Xsig_pred_.col(i) - x_;

    // Angle normalization
    cout<<"Angle normalization"<<endl;
    while(x_diff(3) > M_PI) x_diff(3) -= 2. * M_PI;
    while(x_diff(3) < -M_PI) x_diff(3) += 2. * M_PI;

    Tc += weights_(i) * x_diff * z_diff.transpose();
  }

  // Compute kalman gain K
  cout<<"Compute kalman gain K"<<endl;
  MatrixXd K = Tc * S.inverse();

  // Residual
  cout<<"Residual"<<endl;
  VectorXd z_diff = z - z_pred;

  // Angle normalization
  cout<<"Angle normalization"<<endl;
  while(z_diff(1) > M_PI) z_diff(1) -= 2. * M_PI;
  while(z_diff(1) < -M_PI) z_diff(1) += 2. * M_PI;

  // Update state mean and covariance matrix
  cout<<"Update state mean and covariance matrix"<<endl;
  x_ += K * z_diff;
  P_ -= K * S * K.transpose();

  // Compute NIS for lidar sensor
  cout<<"Compute NIS for lidar sensor"<<endl;
  NIS_lidar_ = z_diff.transpose() * S.inverse() * z_diff;
  cout<< "LIDAR updated!"<< endl;
}

/**
 * Updates the state and the state covariance matrix using a radar measurement.
 * @param {MeasurementPackage} meas_package
 */
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  /**
  TODO:

  Complete this function! Use radar data to update the belief about the object's
  position. Modify the state vector, x_, and covariance, P_.

  You'll also need to calculate the radar NIS.
  */
  // Create matrix for sigma points in measurement space
  MatrixXd Zsig = MatrixXd(n_z_radar, n_sigma_points_);
  
  // Transform sigma points into measurement space
  cout<<"Transform sigma points into measurement space"<<endl;
  for(int i = 0; i < n_sigma_points_; i++){
    // Extract values for better readability
    double p_x = Xsig_pred_(0, i);
    double p_y = Xsig_pred_(1, i);
    double v = Xsig_pred_(2, i);
    double yaw = Xsig_pred_(3, i);

    double v1 = cos(yaw) * v;
    double v2 = sin(yaw) * v;

    // Measurement model
    Zsig(0, i) = sqrt(p_x * p_x + p_y * p_y); // r

    if(fabs(p_y) > 0.001 && fabs(p_x) > 0.001){
      Zsig(1, i) = atan2(p_y, p_x); // phi
    } else {
      Zsig(1, i) = 0.0;
    }
    
    if(fabs(sqrt(p_x * p_x + p_y * p_y)) > 0.001){
      Zsig(2, i) = (p_x * v1 + p_y * v2) / sqrt(p_x * p_x + p_y * p_y); // r_dot
      } else {
      Zsig(2, i) = 0.0;
    }
  }

  // Mean predicted measurement
  cout<<"Mean predicted measurement"<<endl;
  VectorXd z_pred = VectorXd(n_z_radar);
  z_pred.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    z_pred += weights_(i) * Zsig.col(i);
  }

  // Innovation covariance matrix S
  cout<<"Innovation covariance matrix S"<<endl;
  MatrixXd S = MatrixXd(n_z_radar, n_z_radar);
  S.fill(0.0);
  for(int i = 0; i < n_sigma_points_; i++){
    // Residual
    VectorXd z_diff = Zsig.col(i) - z_pred;

    // Angle normalization
    while(z_diff(1) > M_PI) z_diff(1) -= 2. * M_PI;
    while(z_diff(1) < -M_PI) z_diff(1) += 2. * M_PI;
    S += weights_(i) * z_diff * z_diff.transpose();
  }

  // Add measurement noise covariance matrix
  cout<<"Add measurement noise covariance matrix"<<endl;
  R_radar_ << std_radr_ * std_radr_, 0, 0, 0, std_radphi_ * std_radphi_, 0, 0, 0, std_radrd_ * std_radrd_;

  S += R_radar_;

  // Parse radar measurement
  cout<<"Parse radar measurement"<<endl;
  VectorXd z = VectorXd(n_z_radar);
  z <<  meas_package.raw_measurements_[0], meas_package.raw_measurements_[1], meas_package.raw_measurements_[2];

  // Compute cross correlation matrix
  cout<<"Compute cross correlation matrix"<<endl;
  MatrixXd Tc = MatrixXd(n_x_, n_z_radar);
  cout<<"1"<<endl;
  
  Tc.fill(0.0);
  cout<<"2"<<endl;
  for(int i = 0; i < n_sigma_points_; i++){
    cout<<i<<endl;
    // Residual
    cout<<"Residual"<<endl;
    VectorXd z_diff = Zsig.col(i) - z_pred;

    // Angle normalization
    cout<<"Angle normalization 1"<<endl;
    while(z_diff(1) > M_PI) z_diff(1) -= 2. * M_PI;
    while(z_diff(1) < -M_PI) z_diff(1) += 2. * M_PI;

    // State difference
    cout<<"State difference"<<endl;
    VectorXd x_diff = Xsig_pred_.col(i) - x_;

    // Angle normalization
    cout<<"Angle normalization 3"<<endl;
    while(x_diff(3) > M_PI) x_diff(3) -= 2. * M_PI;
    while(x_diff(3) < -M_PI) x_diff(3) += 2. * M_PI;

    cout<<"Tc"<<endl;
    Tc += weights_(i) * x_diff * z_diff.transpose();
  }

  // Compute kalman gain K
  cout<<"Compute kalman gain K"<<endl;
  MatrixXd K = Tc * S.inverse();

  // Residual
  cout<<"Residual"<<endl;
  VectorXd z_diff = z - z_pred;

  // Angle normalization
  cout<<""<<endl;
  while(z_diff(1) > M_PI) z_diff(1) -= 2. * M_PI;
  while(z_diff(1) < -M_PI) z_diff(1) += 2. * M_PI;

  // Update state mean and covariance matrix
  cout<<"Update state mean and covariance matrix"<<endl;
  x_ += K * z_diff;
  P_ -= K * S * K.transpose();

  // Compute NIS for radar sensor
  cout<<"Compute NIS for radar sensor"<<endl;
  NIS_radar_ = z_diff.transpose() * S.inverse() * z_diff;
  cout<<"RADAR updated!"<<endl;
}
